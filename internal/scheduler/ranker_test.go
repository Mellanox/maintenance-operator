...
var _ = Describe("Ranker tests", func() {
	...
	Context("Rank tests", func() {
		It("should rank properly", func() {
			...
			Expect(rankedMap["nm-2"].Rank).To(Equal(scheduler.InProgressRankerWeight + scheduler.LeastPendingRankerMaxWeight - 1))
			Expect(rankedMap["nm-3"].Rank).To(Equal(scheduler.InProgressRankerWeight + scheduler.LeastPendingRankerMaxWeight - 1))
			Expect(rankedMap["nm-4"].Rank).To(Equal(scheduler.InProgressRankerWeight + scheduler.LeastPendingRankerMaxWeight))
			Expect(rankedMap["nm-5"].Rank).To(Equal(scheduler.LeastPendingRankerMaxWeight))
		})
	})
	...
	Context("LeastPendingRanker tests", func() {
		It("several pending maintenances", func() {
			...
			Expect(ranked[1].Rank).To(Equal(scheduler.LeastPendingRankerMaxWeight))
			Expect(ranked[2].Rank).To(Equal(scheduler.LeastPendingRankerMaxWeight - 1))
		...
		It("edgecase: single pending maintenance", func() {
			...
			Expect(ranked[0].Rank).To(Equal(scheduler.LeastPendingRankerMaxWeight))
		})
